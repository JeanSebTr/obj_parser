...
void MeshObj::charger_obj(string nom,MeshObj *first)
{
    ...
    if(first) //Si ce n'est pas la premi�re frame, on demande � celle-ci de nous passer ses mat�riaux et sa texture (et elle accepte ^^ )
    ...
    if(fichier)
    {
        ...
        {
            if(ligne[0]=='v') //Coordonn�es de points (vertex, texture et normale)
            {
                if(ligne[1]==' ') //Vertex
                {
            ...
            else if(ligne[0]=='f') //Indice faces
            {
                ...
                for(int i=0;i<(ndonnees==3?3:4);i++) //On aurait tr�s bien pu mettre i<ndonnees mais je veux vraiment limiter � 3 ou 4
                {
            ...
            else if(ligne[0]=='m'&&first==NULL)//fichier MTL et si c'est la premi�re frame (comme �a on ne charge pas plusieurs fois le m�me MTL et la m�me texture)
